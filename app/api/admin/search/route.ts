  ...
  try {
    ...
    const response = await getOpenAI().chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an intelligent search engine for a participant directory. Given a query and numbered participant profiles, return ALL matching profile indices ranked by relevance.
...
3. **Implicit roles** (IMPORTANT): Infer what someone likely does based on context clues:
   - Someone at a VC/PE firm (Sequoia, a16z, etc.) → investor, even if title says "Associate"
...
4. **Intent-based queries**: Think about what the user actually needs:
   - "People to meet for fundraising" → VCs, angels, startup advisors, people who've raised before
   - "Potential customers for my SaaS" → business leaders, ops/growth/marketing heads
   - "Interesting people to talk to" → diverse, influential, or unique profiles
5. **Education**: match institution names, even abbreviated (e.g. "MIT" = Massachusetts Institute of Technology)
6. **Location**: match city, country, or region (e.g. "European" matches Germany, France, etc.)
7. **Status queries**: "completed" or "pending" should match survey completion status if tags include it
8. **Be generous**: when in doubt, include the person. Better to show a slightly broader set than miss someone relevant.
